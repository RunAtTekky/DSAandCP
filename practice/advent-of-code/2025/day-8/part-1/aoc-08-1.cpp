...
DSU create_DSU(vector<vector<ll>>& junction_distances, vector<vector<ll>>& coord) {
    ll n = coord.size();
    DSU dsu(n+1);
...
ll find_ans(vector<vector<ll>>& junction_distances, vector<vector<ll>>& coord) {
    // TODO: Get answer
    DSU dsu = create_DSU(junction_distances, coord);

    auto junction_sizes = dsu.get_sizes();

    sort(junction_sizes.begin(), junction_sizes.end(), greater<ll>());

    for (auto &ele : junction_sizes) cout << ele << " ";
    cout << "\n";
    

    ll ans = junction_sizes[0] * junction_sizes[1] * junction_sizes[2];
    return ans;
}
